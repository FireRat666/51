...
export class FireScreen {
    ...
    async _createAllUI() {
        ...
        const BUTTON_CONFIGS = {
            ...
            pageBack: { icon: p['icon-direction-url'], position: new BS.Vector3(-0.5, TButPos, 0), color: p['button-color'], clickHandler: () => { this._runBrowserActions(JSON.stringify({ "actions": [{ "actionType": "goback" }] })); this._updateButtonColor(this.uiButtons.pageBack, p['button-color']); this._dispatchButtonClickEvent("Back", 'Back Clicked!'); } },
            ...
            pageForward: { icon: p['icon-direction-url'], position: new BS.Vector3(-0.38, TButPos, 0), color: p['button-color'], clickHandler: () => { this._runBrowserActions(JSON.stringify({ "actions": [{ "actionType": "goforward" }] })); this._updateButtonColor(this.uiButtons.pageForward, p['button-color']); this._dispatchButtonClickEvent("Forward", 'Forward Clicked!'); }, rotation: new BS.Vector3(0, 0, 180) },
