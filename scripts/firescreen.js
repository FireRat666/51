...
function keepsoundlevel() {
  ...
  volInterval = setInterval(function() {
    for (let i = 1; i <= window.NumberofBrowsers; i++) {
			...
			`document.querySelectorAll('video, audio').forEach((elem) => elem.volume=${volume});document.querySelector('.html5-video-player') ? document.querySelector('.html5-video-player').setVolume(${Math.round(volume * 100)}) : null;`, }, ]);
...
init: function () { this.el.addEventListener("click", () => {  
  ...
  browserElement.components["sq-browser"].runActions([{ actionType: "runscript", strparam1: `document.querySelectorAll('video, audio').forEach((elem) => elem.volume=${volume});document.querySelector('.html5-video-player') ? document.querySelector('.html5-video-player').setVolume(${firepercent}) : null;`,}]);
      ...
      if (data.firevolume) { const fireVolume = Number(parseFloat(data.firevolume).toFixed(2));
        document.querySelectorAll('.firescreenc').forEach(element => { element.setAttribute("volumelevel", fireVolume);
          element.components["sq-browser"].runActions([
            { actionType: "runscript", strparam1: `document.querySelectorAll('video, audio').forEach(elem => elem.volume=${fireVolume}); document.querySelector('.html5-video-player') ? document.querySelector('.html5-video-player').setVolume(${(fireVolume * 100).toFixed(0)}) : null;` }
          ]);
        });
      };
  ...
  class handButtonCrap{
    ...
    updateVolume(element, volume, index) {
      ...
      element.components["sq-browser"].runActions([
        ...
        { actionType: "runscript", strparam1: `document.querySelector('.html5-video-player') ? document.querySelector('.html5-video-player').setVolume(${firePercent}) : null;` }
      ]);
    };
